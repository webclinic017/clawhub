...
export const getStatsInternal = internalAction({
  ...
  handler: async (ctx) => {
    ...
    while (!done) {
      const page = await ctx.runQuery(internal.skills.getStatsPageInternal, {
        cursor,
      })

      total += page.total
      for (const [k, v] of Object.entries(page.byStatus)) {
        byStatus[k] = (byStatus[k] ?? 0) + v
      }
      for (const [k, v] of Object.entries(page.byReason)) {
        byReason[k] = (byReason[k] ?? 0) + v
      }
      for (const [k, v] of Object.entries(page.byFlags)) {
        byFlags[k] = (byFlags[k] ?? 0) + v
      }
    ...
    const highlighted = await ctx.runQuery(internal.skills.getHighlightedCountInternal, {})
